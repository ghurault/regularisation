...
SNR=2 # 1,2,4 # = sd(Y)/sigma

# Plot value for beta_true
...
plot_beta <- function(beta,vs="Coefficient",count=FALSE,smooth=FALSE){
  library(ggplot2)
  library(reshape2)
  cbbPalette <- c("#000000", "#E69F00", "#56B4E9", "#009E73", "#F0E442", "#0072B2", "#D55E00", "#CC79A7")
  
  tmp=melt(beta,id.vars="True",variable.name = "Method",value.name = "Coefficient")
  tmp$Error=tmp$Coefficient-tmp$True
  
  p=ggplot(data=tmp,aes_string(x="True",y=vs,colour="Method"))
  
  if (count){p=p+geom_count()}else{p=p+geom_point(size=2)}
  if (smooth){p=p+geom_smooth(method="loess",se=TRUE,level=1-exp(-10))}
  p=p+
    geom_path()+
...
performance_chart <- function(df,ref){
  ...
  a=with(tmp,max(Mean+SE))
  
  p=ggplot(data=tmp,aes(x=Method,y=Mean,ymin=Mean-SE,ymax=Mean+SE))+
    geom_bar(stat="identity")+geom_errorbar()+
    geom_text(aes(y=Mean+SE+a*0.03,label=paste(signif(Mean,3),"±",signif(SE,2))))+
    scale_y_continuous(expand = c(0,0),limits = c(0,a*1.1))+
    labs(x="",y="MSE")+
...
if (condition=="3"){
  beta_true=(0:(D-1))^2;beta_true=beta_true/max(beta_true)*beta_max # increasing beta (more around 0)
  K=sum(beta_true!=0)
}
...
beta$Lasso=as.numeric(coef(lasso,s="lambda.1se")[-1])
...
## Prediction plots
pl2=lapply(colnames(pred)[-1],function(x){
...
pos=1:(4*nrow(s));tmp$Position=pos[pos%%4>0] # add spacing between betas
